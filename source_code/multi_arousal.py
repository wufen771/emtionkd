...
train_labels_arousal = labels_arousal[:train_size]
val_labels_arousal = labels_arousal[train_size:train_size + val_size]
test_labels_arousal = labels_arousal[train_size + val_size:]

# 转换为 one-hot 编码
train_labels_arousal = tf.keras.utils.to_categorical(train_labels_arousal, num_classes=2)
val_labels_arousal = tf.keras.utils.to_categorical(val_labels_arousal, num_classes=2)
test_labels_arousal = tf.keras.utils.to_categorical(test_labels_arousal, num_classes=2)
...
multi_modal_model.fit([train_eeg, train_gsr], 
                      train_labels_arousal, epochs=20, 
                      batch_size=32, 
                      validation_data=([val_eeg, val_gsr], val_labels_arousal),
                      callbacks=[early_stopping, reduce_lr])

# 评估多模态模型
multi_modal_results = multi_modal_model.evaluate([test_eeg, test_gsr], test_labels_arousal)
...
true_test_labels = np.argmax(test_labels_arousal, axis=1)
...
# 训练单模态模型
gsr_model.fit([train_gsr, train_labels_arousal, soft_labels_train, att_features_train], 
              train_labels_arousal, epochs=20, batch_size=32,
              validation_data=([val_gsr, val_labels_arousal, soft_labels_val, att_features_val], 
                               val_labels_arousal),
              callbacks=[early_stopping, reduce_lr])

# 评估单模态模型
results = gsr_model.evaluate([test_gsr, test_labels_arousal, soft_labels_test, att_features_test], test_labels_arousal)
print("Evaluation Results:", results)

# 获取单模态模型的预测结果
gsr_predictions = gsr_model.predict([test_gsr, test_labels_arousal, soft_labels_test, att_features_test])
